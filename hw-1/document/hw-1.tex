...
\begin{itemize}
    ...
    \begin{align*}
        ...
        &= 435.9\ \mathrm{kW}
    \end{align*}
    \item \textbf{CLI Commands:} See CLI\_Commands.md for detailed property lookup commands
\end{itemize}
...
\textbf{Given:}
\begin{itemize}
    \item Mass flow rate: $\dot{m} = 2.5\ \mathrm{kg/s}$
    \item Inlet conditions: $P_1 = 3\ \mathrm{MPa}$, $T_1 = 400^\circ\mathrm{C}$ (superheated steam)
    \item Outlet pressure: $P_2 = 100\ \mathrm{kPa}$
    \item Isentropic efficiency: $\eta_s = 0.85$
    \item Process: Adiabatic expansion ($\dot{Q} = 0$), negligible kinetic/potential energy changes
\end{itemize}
...
\textbf{Solution}
\begin{enumerate}
    \item \textbf{Look up saturated steam properties at $P = 200$ kPa:}
    \begin{itemize}
        \item $s_f = 1.530$ kJ/kg$\cdot$K (saturated liquid entropy)
        \item $s_g = 7.127$ kJ/kg$\cdot$K (saturated vapor entropy) 
...
\textbf{Solution}
\begin{enumerate}
    \item \textbf{Find initial state properties at 150 kPa saturated liquid:}
    \begin{itemize}
        \item $h_1 = h_f = 467.1$ kJ/kg
        \item $s_1 = s_f = 1.434$ kJ/kg$\cdot$K
        \item $\rho_1 = 949.9$ kg/m$^3$
    ...
    \begin{itemize}
        \item Reference: $h_g = 2693.1$ kJ/kg (saturated vapor enthalpy)
        \item Since $h_1 = 467.1 < h_2 = 840.1 < h_g = 2693.1$ $\rightarrow$ \textbf{two-phase mixture}
        \item Quality: $x_2 = 0.168$ (16.8\% vapor)
        \item Final entropy: $s_2 = 2.404$ kJ/kg$\cdot$K
    \end{itemize}
...
\begin{itemize}
    \item \textbf{Iterative Solution:} Using property\_solver.py to find $T_5$ where $s_5 = 4468.7$ J/kg$\cdot$K at $P_5 = 100$ kPa
    \item \textbf{Result:} $T_5 = 529.7$ K
\end{itemize}

\textbf{Enthalpy Calculations:} Using CoolProp CLI for property lookup:
\begin{align*}
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Analysis Scope:} Combined receiver-engine efficiency optimization
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Carnot efficiency increases with $T_H$: $\eta_{Carnot} = 1 - T_L/T_H$
        \item Receiver efficiency decreases with $T_H$: $\eta_{receiver} = 1 - \sigma T_H^4/(C \cdot I)$
        \item Optimal point balances these competing effects
        \item Higher concentration shifts optimum to higher temperatures and efficiencies
    \end{itemize}
\end{enumerate}
